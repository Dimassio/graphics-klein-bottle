...
MeshPtr makeKleinBottle( const float radius, const size_t N )
{
	...
	for( size_t i = 0; i < N; i++ ) { // [-pi, pi]
		...
		for( size_t j = 0; j < N; j++ ) { // [0, 2pi]
			...
			normals.push_back( normal( u1, v1 ) );//glm::normalize( point1 ) ); //
			normals.push_back( normal( u2, v1 ) );//glm::normalize( point2 ) ); //
			normals.push_back( normal( u2, v2 ) );//glm::normalize( point3 ) ); //

			normals.push_back( normal( u1, v1 ) );//glm::normalize( point4 ) ); //
			normals.push_back( normal( u2, v2 ) );//glm::normalize( point2 ) ); //
			normals.push_back( normal( u1, v2 ) );//glm::normalize( point3 ) ); //
